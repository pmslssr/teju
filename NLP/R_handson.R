...
#Explorative data analysis (statistics)
mean(mtcars$mpg)
median(mtcars$mpg)
table(df$Rain)
...
b$stats
quantile(mtcars$mpg, probs = seq(0,1,0.02))

boxplot(mtcars$mpg~mtcars$cyl,main="Boxplot b/w mpg and cyl", xlab ="Cyl",ylab = "mpg",col = c("orange","blue","green"))
b$stats
quantile(mtcars$mpg, probs = seq(0,1,0.02))
...
library(lattice)
bwplot(mtcars$mpg~mtcars$cyl,main="Boxplot b/w mpg and cyl", xlab ="Cyl",ylab = "mpg",col = c("orange","blue","green"))
#a = bwplot(iris$Petal.Length ~)
...
#to know missing values in the table
...
airquality$Ozone[airquality$Ozone == mean(airquality$Ozone)]<-'NA'
airquality

#probability of getting 2 heads when a coin is tossed for 5 times is given by
dbinom(2,5,0.5)

#probability of getting upto 2 heads when a coin is tossed for 5 times is given by adding the following three
dbinom(0,5,0.5)
dbinom(1,5,0.5)
dbinom(2,5,0.5)
